...
class Comm
{
    ...
    {
        $count_type = ['num', 'quan', 'post', 'page', 'page_info'];
    ...
    {
        $result = ['error' => '未知错误', 'list' => []];
        $user_dev = H::user_dev();
        //可以获取的数据类型
        $params['type'] = isset($params['type']) ? $params['type'] : '';
        $type_array = ['num', 'quan', 'post', 'page', 'info'];
        //不在允许范围内，返回空数组
    ...
    public function show_top_hot()
    {
        ...
        foreach ($list as $k => $v)
        {
            ...
            foreach ($v as $v1)
            {
                ...
                if ($k == 1)
                {
                    $surl = DOMAIN_WWW . "/app/qun/list.php?kw=" . urlencode($v1['qword']);
                }
                if ($k == 2)
                {
                    $surl = DOMAIN_WWW . "/app/quan/list.php?kw=" . urlencode($v1['qword']);
                }
                if ($k == 3)
                {
                    $surl = DOMAIN_WWW . "/app/quan/list.post.php?kw=" . urlencode($v1['qword']);
                }
                if ($k == 4)
                {
                    $surl = DOMAIN_WWW . "/app/home/list.php?kw=" . urlencode($v1['qword']);
                }
    ...
    public function show_search()
    {
            ...
            <div class="sel_box" onclick="select_single(event,this);return false;">
                <a href="javascript:void(0);" class="txt_box" id="txt_box">
                    ...
                    <input type="hidden" name="search_type" id="search_type"
                           value="<?php echo($_GET['search_type']); ?>" class="sel_subject_val">
                </a>
        ...
        <input type="text" id="kw" name="kw" value="<?php echo($_GET['kw']); ?>" class="search_txt"
               onfocus="document.getElementById('lab_txt').style.display = 'none'"
               onblur="this.value == '' ?  document.getElementById('lab_txt').style.display='block' : document.getElementById('lab_txt').style.display = 'none'"
               onkeyup="if(event.keyCode==13) do_search();"/>
        <label for="kw" id="lab_txt"><?php if ($_GET['kw'] == '')
            { ?>请输入要搜索的关键词<?php } ?></label>
    ...
    public function show_quit()
    {
        ...
        <div class="ft_quit">
            ...
            <ul class="fq_form" id="fq_form">
                <li>
                    <span><input type="checkbox" id="fq_cbx1" class="chk_list"
                                 value="没有我需要的内容"/><label for="fq_cbx1">没有我需要的内容</label></span>
                    <span><input type="checkbox" id="fq_cbx2" class="chk_list"
                                 value="对弹层提醒无法再爱"/><label for="fq_cbx2">对弹层提醒无法再爱</label></span>
                </li>
                <li>
                    <span><input type="checkbox" id="fq_cbx3" class="chk_list"
                                 value="功能体验屋里吐槽"/><label for="fq_cbx3">功能体验屋里吐槽</label></span>
                    <span><input type="checkbox" id="fq_cbx4" class="chk_list"
                                 value="我就钟爱非登录首页"/><label for="fq_cbx4">我就钟爱非登录首页</label></span>
                </li>
                <li class="fq_txt">
                    <p>更多槽点吐这里哦：</p>
    ...
    public function show_sidebar_wap()
    {
        ...
        <section id="sidebar_x" class="sidebar_x">
            <div class="cnt_nav">
                <a href="javascript:void(0);" onclick="show_send_message('','');"
                   class="inbox"><i></i>发私信</a>
                <a href="<?php echo DOMAIN_USER; ?>/app/user/message.php?type=inbox"
                   class="sibox"><i></i>收件箱</a>
                <a href="<?php echo DOMAIN_USER; ?>/app/user/message.php?type=outbox"
                   class="outbox"><i></i>发件箱</a>
            </div>
            <!--
                <div class="cnt_nav">
            ...
            <div class="cnt_nav">
                <a href="<?php echo DOMAIN_USER; ?>/app/api/common.ver1.0.php?m=logout"
                   class="exit"><i></i>安全退出</a>
            </div>
        </section>
        <?php
    }
    ...
    public function show_nav_wap()
    {
        ...
        <header>
            <span class="hd_left l">
                ...
                { ?>
                    <a class="log_ico"
                       href="javascript:void(0);"> <!-- href="<?php echo DOMAIN_USER; ?>/app/user/index.php"-->
                        <img src="<?php echo $session_user['avatar']; ?>"
                             onerror="$(this).remove();"/>
                    </a>
    ...
    public function show_bot_wap()
    {
        ...
        ?>
        <footer>
            <a href="<?php echo DOMAIN_WWW . '/app/quan/picked.quan.php' ?>"
               class="i_qun"><i></i><span>产品</span></a>
            <a href="<?php echo DOMAIN_WWW . '/app/quan/list.php' ?>"
               class="i_quan"><i></i><span>社区</span></a>
            <a href="<?php echo DOMAIN_WWW . '/app/home/list.php' ?>" class="i_shang"><i></i><span>微商</span></a>
            <a href="<?php echo DOMAIN_WWW . '/app/search/' ?>"
               class="i_search"><i></i><span>搜索</span></a>
        </footer>
        <div style="display:none;">
            <script src="http://s11.cnzz.com/stat.php?id=1254701924&web_id=1254701924"
    ...
    {
        $arr = [
            ['tag' => 'qun', 'name' => '微信群', 'url' => DOMAIN_WWW . '/app/qun/', 'target' => '_self'],
            ['tag' => 'quan', 'name' => '微商社区', 'url' => DOMAIN_WWW . '/app/quan/', 'target' => '_self'],
            ['tag' => 'home', 'name' => '微商团队', 'url' => DOMAIN_WWW . '/app/home/', 'target' => '_self'],
            ['tag' => 'fx', 'name' => '加粉神器', 'url' => DOMAIN_TOOL . '/', 'target' => '_blank'],
        ];
        foreach ($arr as $k => $v)
        {
            $style = '';
            if (strpos($_SERVER['PHP_SELF'], $v['tag']))
            {
                $style = 'current';
            }
    ...
    public function show_top_nav()
    {
        ...
        <div class="header_top">
            <div class="WB_global_nav">
                <div class="gn_header">
                    <div class="logo_im"><a href="<?php echo DOMAIN_WWW; ?>"><img
                                    src="<?php echo(DOMAIN_CSS); ?>/www/images/logo1.png"></a></div>
                    ...
                    <div class="gn_right r">
                        ...
                        <div class="r_dl r">
                                ...
                                <a href="<?php echo DOMAIN_USER; ?>" class="deng">
                                    <i>
                                        <img onerror="$(this).remove();"
                                             src="<?php echo $session_user['thumb_avatar']; ?>"/>
                                    </i>
                                </a>
                                <a href="<?php echo DOMAIN_USER; ?>/app/api/common.ver1.0.php?m=logout"
                                   class="zu" id="quit_s">
                            ...
                            { ?>
                                ...
                                <a href="<?php echo DOMAIN_USER; ?>/app/user/register.php"
                                   class="zu">
                                    注册
                                </a>
                            <?php } ?>
    ...
    public function show_bot()
    {
        ...
        <div class="footer_wrap" style="height:320px;">
            <div class="footer">
                ...
                <p class="line-t-20"></p>
                <span>© 2012-2015 HuiWeiShang.COM 版权所有 赣ICP备10004530号 <font style="font-size:11px;">Processed in <?php echo(number_format((H::getmicrotime() - $time_start) / 1000, 2)); ?>
                        second(s)</font></span>
                <p class="line-t-20"></p>
                <a href="http://webscan.360.cn/index/checkwebsite/url/www.huiweishang.com"><img
                            border="0"
                            src="http://img.webscan.360.cn/status/pai/hash/2d575b3e43490f22a172f59d25c137ae"/></a>
            </div>
        ...
        <div id="show_send_message_div" style="display:none;">
            ...
            <ul class="lr_form release_form">
                <li>
                    <?php if (USER_DEV == 'pc')
                    {
                        echo '<span class="tit_span"><i>*</i>收件人：</span>';
                    } ?><input type="text" class="lr_ipt" id="uid_to"
                               value="<?php echo isset($_GET['uid_to']) ? urldecode($_GET['uid_to']) : ''; ?>"
                               placeholder="收件人"/>
                ...
                <!--<li>
                    <?php if (USER_DEV == 'pc')
                {
                    echo '<span class="tit_span"><i>*</i>标题：</span>';
                } ?><input type="text" class="lr_ipt" id="title" value="<?php echo isset($_GET['title']) ? urldecode($_GET['title']) : ''; ?>" placeholder="标题"/>
                </li>-->
                <li class="txt_li" style="height:100px;">
                    <?php if (USER_DEV == 'pc')
                    {
                        echo '<span class="tit_span"><i>*</i>内容：</span>';
                    } ?>
                    <textarea id="message" class="lr_ipt" style="max-width:400px;height:60px"
                              placeholder="内容"></textarea>
                </li>
                <li>
                    <?php if (USER_DEV == 'pc')
                    {
                        echo '<span class="tit_span">&nbsp;</span>';
                    } ?>
                    <div class="quanzi_btn l fl_none">
                        <p class="line-t-6"></p>
                        <a href="javascript:void(0);" class="replay_btn" id="saveinfo"
                           onclick="send_message()">发送</a>
                    </div>
                </li>
            </ul>
        </div>
    ...
    {
        ?>
        <div id="site_share_div" class="bdsharebuttonbox"><a href="#" class="bds_more"
                                                             data-cmd="more"></a><a title="分享到QQ好友"
                                                                                    href="#"
                                                                                    class="bds_sqq"
                                                                                    data-cmd="sqq"></a><a
                    ...
                    title="分享到人人网" href="#" class="bds_renren" data-cmd="renren"></a><a
                    title="分享到豆瓣网" href="#" class="bds_douban" data-cmd="douban"></a></div>
    ...
    {
        if (in_array($type, ['num', 'page', 'quan', 'post', 'info']))
        {
    ...
    public function m__save_message()
    {
        ...
        if ($_POST['uid_to'] == '')
        {
            H::error_show('{"code":"1","msg":"请填写收件人","id":"uid_to"}');
        }
        //if($_POST['title'] == '') H::error_show('{"code":"1","msg":"请填写标题","id":"title"}');
        if ($_POST['message'] == '')
        {
            H::error_show('{"code":"1","msg":"请填写内容","id":"message"}');
        }
        ...
        if (count($tmp))
        {
            H::error_show('{"code":"1","msg":"收件人 “' . implode('”,“', $tmp) . '” 不存在","id":"uid_to"}');
        }
    ...
    public function m__get_code_son()
    {
        ...
        if (count($T->trees($_POST['code_id'])) <= 1)
        {
            die ('{"code":"0","msg":"拉取数据成功","html":' . H::json_encode_ch($def_html) . '}');
        }
        $tmp_tree = $T->trees($_POST['code_id']);
        if (count($tmp_tree['son']) == 0)
        {
            die ('{"code":"0","msg":"拉取数据成功","html":' . H::json_encode_ch($def_html) . '}');
        }
        ...
        if ($_POST['is_get'])
        {
            $on = 'get_code_son(this,\'' . $_POST['div_id'] . '\',' . $_POST['is_get'] . ')';
        }

        $V->set_fields($_POST['next_id'], $T->get_code_son($_POST['code_id']));
        $V->set_field($_POST['next_id'], ['value' => 0, 'txt' => $_POST['def'], 'txt_color' => '']);
        $str = $V->input_str(['node' => $_POST['next_id'], 'type' => 'select_single', 'default' => 0, 'style' => 'style="width:' . $_POST['width'] . 'px;z-index: 1;"', 'on' => $on]);
        unset($dbm);
    ...
    public function m__save_page_follow()
    {
        ...
        if ($page_uid <= 0)
        {
            H::error_show('{"code":"1","msg":"主页ID不正确"}');
        }
        if ($page_uid == $uid)
        {
            H::error_show('{"code":1,"msg":"请不要关注自己的主页"}');
        }

        $fields = ['uid' => $uid, 'page_uid' => $page_uid];

        if ($ac == 'add')
        {
            $msg = $P->update_page_follow($fields, 'add');
            //关注微商加积分
            $U->update_point(['uid' => $uid, 'code_id' => 11445]);
        }
        else
        {
            $msg = $P->update_page_follow($fields, 'cancel');
            //取消关注微商扣积分
            $U->update_point(['uid' => $uid, 'code_id' => 11446]);
        }
        $U->update_count($uid, ['page']);
    ...
    public function m__save_quan_follow()
    {
        ...
        if ($quan_uid <= 0)
        {
            H::error_show('{"code":"1","msg":"圈子ID出错"}');
        }
        session_open();
        $uid = isset($_SESSION['user']['uid']) ? intval($_SESSION['user']['uid']) : 0;
        session_close();
        ...
        if ($quan_uid == $uid)
        {
            H::error_show('{"code":1,"msg":"请不要关注自己创建的圈子"}');
        }
        $fields = ['uid' => $uid, 'quan_uid' => $quan_uid];
        if ($ac == 'add')
        {
        ...
        $U->update_quan_count($quan_uid, ['quan']);
        H::error_show('{"code":"0","msg":"' . $msg . '"}');
    }
    ...
    function m__user_quit()
    {
        ...
        if ($sugges != '')
        {
            $fields = array_merge($fields, explode(',', $sugges));
        }
        //die(print_r($fields));
        if (count($fields) > 0)
        {
            foreach ($fields as $v)
            {
                ...
                {
                    $dbm->single_insert(DB_DBNAME . ".ws_sugges", ["content" => $v, 'create_time' => time(), 'ctotal' => 1]);
                }
            }
        }
    ...
    public function get_ad($area_id, $is_return = 0)
    {
        ...
        //代码广告
        if ($rs['list'][0]['area_type'] == 'code')
        {
            echo htmlspecialchars_decode($rs['list'][0]['area_html']);
            return;
        }
    ...
    function m__register()
    {
        ...
        {
            if ($k != 'login_pass' && $k != 'login_pass_repeat')
            {
                $_POST[$k] = H::sqlxss($v);
            }
        }
        ...
        if ($verify != '')
        {
            H::error_show('{"code":1,"msg":"' . $verify . '","id":"login_name"}');
        }
        if ($fields['login_name'] == '')
        {
            H::error_show('{"code":1,"msg":"请填写帐号","id":"login_name"}');
        }

        $user = $U->get($fields['login_name'], 'uid', 2);
        if ($user)
        {
            H::error_show('{"code":1,"msg":"帐号已经存在","id":"login_name"}');
        }

        $verify_pass = verify::verify_upass($fields['login_pass']);
        if ($verify_pass != '')
        {
            H::error_show('{"code":1,"msg":"' . $verify_pass . '","id":"login_pass"}');
        }
        /*
        ...
        */
        ...
        if ($verify != '')
        {
            H::error_show('{"code":1,"msg":"请填写正确的手机号码","id":"login_mobile"}');
        }
        ...
        if ($_POST['mcode'] <= 0)
        {
            H::error_show('{"code":1,"msg":"短信验证码错误","id":"login_name"}');
        }
        ...
        if (!$vcode_true)
        {
            die('{"code":1,"msg":"短信验证码错误","id":"mcode"}');
        }


        if ($_POST['receive'] != 1)
        {
            H::error_show('{"code":1,"msg":"请选择同意协议","id":"receive"}');
        }
        ...
        if ($user_autoid)
        {
            ...
            $dbm->single_insert(
                ...
                ['reg_status' => $reg_status, 'uid' => $user_autoid, 'huanxin_upass' => $huanxin_upass]
            );
            ...
            session_close();
            //注册赠送积分
            $U->update_point(['uid' => $user_autoid, 'code_id' => 11438]);
            /*注册写入推荐人*/
            if ($hwsid > 0)
            {
                ...
                if (count($ret['list']) == 1)
                {
                    $rec_user = ['uid' => $user_autoid, 'uid_recommend' => $hwsid, 'create_time' => time()];
                    $dbm->single_insert(DB_DBNAME_USER . ".ws_recommend_reg", $rec_user);
                    //推荐人赠送积分
                    $U->update_point(['uid' => $hwsid, 'code_id' => 11494]);
            ...
            //新用注册赠送200名额
            $dbm->single_insert(
                DB_DBNAME_TOOL . ".ws_fans_stat",
                ['uid' => $user_autoid, 'can_use' => TRADE_INIT, 'rec_total' => 0, 'add_total' => 0]
            );
    ...
    function m__check_code()
    {
        ...
        if (isset($_POST['login_mobile']))
        {
            ...
            if ($verify != '')
            {
                die('{"code":1,"msg":"' . $verify . '","id":"login_mobile"}');
            }
        }
        // 登录用户不验证，注册用户要验证唯一性
        if ($uid > 0)
    ...
    function m__login()
    {
        ...
        {
            H::error_show('{"code":"1","msg":"您已经登录了"}');
        }
        ...
        if (in_array($_POST['http_referer'], $tmp))
        {
            $_POST['http_referer'] = DOMAIN_USER . '/app/user/index.php';
        }
        ...
        if ($_POST['login_name'] == '')
        {
            H::error_show('{"code":"1","msg":"请输入帐号","id":"login_name"}');
        }
        //检查密码是否输入
        if ($_POST['login_pass'] == '')
        {
            H::error_show('{"code":"1","msg":"请输入密码","id":"login_pass"}');
        }
        ...
        if (!$user)
        {
            H::error_show('{"code":"1","msg":"帐号密码不匹配","id":"login_name"}');
        }
        if (in_array($user['login_group'], [1, 3, 4]))
        {
            H::error_show('{"code":"1","msg":"系统帐号禁止登录","id":"login_name"}');
        }
        if ($user['login_status'] != 0)
        {
            H::error_show('{"code":"1","msg":"账号被禁用","id":"login_name"}');
        }
